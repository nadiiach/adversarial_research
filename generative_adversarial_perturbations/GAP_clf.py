...
from generative_adversarial_perturbations.material.models.generators \
    import ResnetGenerator, weights_init
from generative_adversarial_perturbations.data \
    import get_training_set, get_test_set
...
parser.add_argument('--batchSize', type=int,
    default=30, help='training batch size')
parser.add_argument('--testBatchSize', type=int,
    default=16, help='testing batch size')
parser.add_argument('--nEpochs', type=int,
    default=10, help='number of epochs to train for')
parser.add_argument('--ngf', type=int,
    default=64, help='generator filters in first conv layer')
parser.add_argument('--optimizer', type=str,
    default='adam', help='optimizer: "adam" or "sgd"')
parser.add_argument('--lr', type=float,
    default=0.0002, help='Learning Rate. Default=0.002')
parser.add_argument('--beta1', type=float,
    default=0.5, help='beta1 for adam. default=0.5')
parser.add_argument('--threads', type=int,
    default=4, help='number of threads for data loader to use')
parser.add_argument('--seed', type=int,
    default=123, help='random seed to use. Default=123')
parser.add_argument('--MaxIter', type=int,
    default=50, help='Iterations in each Epoch')
parser.add_argument('--MaxIterTest', type=int,
    default=160, help='Iterations in each Epoch')
parser.add_argument('--mag_in', type=float,
    default=10.0, help='l_inf magnitude of perturbation')
parser.add_argument('--expname', type=str,
    default='tempname', help='experiment name, output folder')
parser.add_argument('--checkpoint', type=str,
    default='', help='path to starting checkpoint')
parser.add_argument('--foolmodel', type=str, default='incv3',
    help='model to fool: "incv3", "vgg16", or "vgg19"')
...
parser.add_argument('--target', type=int, default=-1,
    help='target class: -1 if untargeted, 0..999 if targeted')
parser.add_argument('--gpu_ids', help='gpu ids: e.g. 0 or 0,1 or 1,2.',
    type=str, default='0')
parser.add_argument('--path_to_U_noise', type=str, default='',
    help='path to U_input_noise.txt (only needed for universal)')
parser.add_argument('--explicit_U', type=str, default='',
    help='Path to a universal perturbation to use')
...
if opt.mode == 'train':
    train_set = torchvision.datasets.ImageFolder(root=opt.imagenetTrain,
                                transform = data_transform)
    training_data_loader = DataLoader(dataset=train_set,
                num_workers=opt.threads, batch_size=opt.batchSize, shuffle=True)

test_set = torchvision.datasets.ImageFolder(root=opt.imagenetVal,
                                            transform=data_transform)
...
if not opt.explicit_U:
    ...
    netG = ResnetGenerator(3, 3, opt.ngf, norm_type='batch', act_type='relu',
                           gpu_ids=gpulist)
    ...
    if opt.perturbation_type == 'universal':
        ...
        noise_tr = torch.from_numpy(im_noise_tr)\
            .type(torch.FloatTensor).cuda(gpulist[0])

        im_noise_te = np.tile(im_noise, (opt.testBatchSize, 1, 1, 1))
        noise_te = torch.from_numpy(im_noise_te)\
            .type(torch.FloatTensor).cuda(gpulist[0])
...
def train(epoch):
    ...
    for itr, (image, _) in enumerate(training_data_loader, 1):
        ...
        if opt.optimizer == 'sgd':
            ...
            optimizerG = optim.SGD(netG.parameters(), lr=opt.lr/lr_mult,
                                   momentum=0.9)
        ...
        for cii in range(3):
            recons[:, cii, :, :] = recons[:, cii, :, :].clone()\
                .clamp(image[:, cii, :, :].min(), image[:, cii, :, :].max())
        ...
        print("===> Epoch[{}]({}/{}) loss: {:.4f}"
              .format(epoch, itr, len(training_data_loader), loss.item()))
...
def test():
    ...
    if opt.perturbation_type == 'universal':
        if opt.explicit_U:
            ...
            U_loaded = U_loaded.expand(opt.testBatchSize, U_loaded.size(1),
                                       U_loaded.size(2), U_loaded.size(3))
    ...
    for itr, (image, class_label) in enumerate(testing_data_loader):
        ...
        if itr % 50 == 1:
            ...
            for c2 in range(3):
                recons[:,c2,:,:] = \
                    (recons[:,c2,:,:] * stddev_arr[c2]) + mean_arr[c2]
                image[:,c2,:,:] = \
                    (image[:,c2,:,:] * stddev_arr[c2]) + mean_arr[c2]
                delta_im_temp[:,c2,:,:] = \
                    (delta_im[:,c2,:,:] * stddev_arr[c2]) + mean_arr[c2]
            if not os.path.exists(opt.expname):
                os.mkdir(opt.expname)

            post_l_inf = (recons - image[0:recons.size(0)]).abs().max() * 255.0
            print("Specified l_inf:", mag_in,
                  "| maximum l_inf of generated perturbations: %.2f"
                  % (post_l_inf.item()))

            torchvision.utils.save_image(recons,
                            opt.expname+'/reconstructed_{}.png'.format(itr))
            torchvision.utils.save_image(image,
                            opt.expname+'/original_{}.png'.format(itr))
            torchvision.utils.save_image(delta_im_temp,
                            opt.expname+'/delta_im_{}.png'.format(itr))
            print('Saved images.')
    ...
    print('Accuracy of the pretrained network on original images: %.2f%%'
          % (100.0 * float(correct_orig) / float(total)))
    if opt.target == -1:
        print('Fooling ratio: %.2f%%' % (100.0 * float(fooled) / float(total)))
    else:
        print('Top-1 Target Accuracy: %.2f%%'
              % (100.0 * float(fooled) / float(total)))
...
def normalize_and_scale(delta_im, mode='train'):
    ...
    for c in range(3):
        delta_im[:, c, :, :] = \
            (delta_im[:, c, :, :].clone() - mean_arr[c]) / stddev_arr[c]
    ...
    for i in range(bs):
        ...
        for ci in range(3):
            ...
            delta_im[i, ci, :, :] = \
                delta_im[i, ci, :, :].clone() \
                * np.minimum(1.0, mag_in_scaled_c / l_inf_channel.cpu().numpy())
...
def checkpoint_dict(epoch):
    ...
    net_g_model_out_path = opt.expname + "/netG_model_epoch_{}_".format(epoch) \
                           + task_label + "_{}.pth"\
                               .format(test_fooling_history[epoch-1])

    if opt.perturbation_type == 'universal':
        u_out_path = opt.expname + "/U_out/U_epoch_{}_".format(epoch) \
                     + task_label + "_{}.pth".format(
    ...
    if test_fooling_history[epoch-1] > best_fooling:
        ...
        print("No improvement:", test_fooling_history[epoch-1],
              "Best:", best_fooling)
