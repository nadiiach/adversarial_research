...
for epoch in range(args.epochs):
    ...
    for i in range(len(train_loader_vgg16_19_res152)):
        ...
        if args.attack_type == 'noise':
            adv1 = netG(noise_tensor_vgg16_19_res152_tr)
            adv2 = netG(noise_tensor_others_tr)
        else:
            adv1 = netG(img1)
            adv2 = netG(img2)

        # Projection
        adv1 = utils.projection(adv1, img1, eps)
        adv2 = utils.projection(adv2, img2, eps)
        ...
        if args.target == -1:
            ...
            loss = 0
            for adv_out, img_out, label in zip(advs_logits_out,
                                               imgs_logits_out, clean_labels):
    ...
    discrimins = "_".join(discriminators_size_larger.values() + \
                          discriminators_size_smaller.values())
