...
def universal_adversarial_perturbation(data_list, model, device, xi=10, delta=0.2, max_iter_uni = 10, p=np.inf, 
    ...
    :param max_iter_uni: optional other termination criterion
    (maximum number of iteration, default = 10*num_images)
    :param p: norm to be used (default = np.inf)
    :param num_classes: For deepfool: num_classes
    (limits the number of classes to test against, by default = 10)
    :param overshoot: For deepfool: used as a termination criterion to
    prevent vanishing updates (default = 0.02).
    :param max_iter_df:For deepfool: maximum number of iterations for
    deepfool (default = 10)
    :param t_p:For deepfool: truth perentage, for how many flipped labels
    in a batch.(default = 0.2)
    ...
    while fooling_rate < 1-delta and itr < max_iter_uni:
        ...
        for k,name in enumerate(pbar):
            ...
            if (k%10 == 0):
                pbar.set_description('Norm of v: '+str(torch.norm(v).detach()
                                                       .cpu().numpy()))
